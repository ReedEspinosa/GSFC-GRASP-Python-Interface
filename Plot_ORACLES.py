...
def PltGRASPoutput(RsltDict1, RsltDict2,file_name,PixNo,nkernel=1):
    ...
    # Plot the fit and measured I data
        ...
        axs[2, nwav].plot(Spheriod ['sca_ang'][:,nwav], Spheriod ['meas_PoI'][:,nwav],color = "k", lw = 1, label="meas")
        axs[2, nwav].plot(Spheriod ['sca_ang'][:,nwav], Spheriod ['fit_PoI'][:,nwav], color =color_sph, ls = '--', label="fit sph")
      
        axs[2, nwav].fill_between(Spheriod ['sca_ang'][:,nwav],(Spheriod ['meas_PoI'][:,nwav]), (Spheriod ['meas_PoI'][:,nwav])*1.03,color = 'r', alpha=0.2,ls = "--", label="+3%")
        axs[2, nwav].fill_between(Spheriod ['sca_ang'][:,nwav],(Spheriod ['meas_PoI'][:,nwav]), (Spheriod ['meas_PoI'][:,nwav])*0.97,color = "b", alpha=0.2,ls = "--", label="-3%")
        ...
        axs[2, nwav].plot(Hex['sca_ang'][:,nwav],Hex['fit_PoI'][:,nwav],color = color_tamu , lw = 2,ls = "dashdot", label = "fit Hex") 
