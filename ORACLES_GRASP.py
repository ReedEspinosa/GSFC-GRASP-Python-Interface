...
for i in range(1):
    PixNo = 1000  #Pixel no of Lat,Lon that we are interested
    ...
    height = 200 
    PixNo =   1250
    HSRL_sphrod = HSLR_run("sphro",HSRLfile_path,HSRLfile_name,PixNo,height)
    ...
    #     PixNo = PixNo +1  #Pixel no of Lat,Lon that we are interested
    ...
    # PltGRASPoutput(rslts_Sph, rslts_Tamu)
        ...
        # axs[0, 1].plot(rslts_Sph[0]['sca_ang'][:,nwav], rslts_Sph[0]['meas_I'][:,nwav]*1.03, ls = "--",label="+3%")
        # axs[0, 1].plot(rslts_Sph[0]['sca_ang'][:,nwav], rslts_Sph[0]['meas_I'][:,nwav]*0.97, ls = "--",label="+3%")
        ...
        # axs[1, 0].plot(rslts_Sph[0]['sca_ang'][:,nwav], rslts_Sph[0]['fit_PoI'][:,nwav]/rslts_Sph[0]['fit_I'][:,nwav], label="fit sph")
        # axs[1, 0].plot(rslts_Sph[0]['sca_ang'][:,nwav], rslts_Sph[0]['meas_PoI'][:,nwav]/rslts_Sph[0]['fit_I'][:,nwav], label="meas sph")
        # axs[1, 0].plot(rslts_Tamu[0]['sca_ang'][:,nwav],rslts_Tamu [0]['fit_PoI'][:,nwav]/rslts_Sph[0]['fit_I'][:,nwav], label = "tamu") 
        # axs[1, 0].plot(rslts_Sph[0]['sca_ang'][:,nwav], (rslts_Sph[0]['meas_PoI'][:,nwav]/rslts_Sph[0]['fit_I'][:,nwav])*1.03,ls = "--", label="+3%")
        # axs[1, 0].plot(rslts_Sph[0]['sca_ang'][:,nwav], (rslts_Sph[0]['meas_PoI'][:,nwav]/rslts_Sph[0]['fit_I'][:,nwav])*0.97,ls = "--", label="-3%")
        ...
        # print(rslts_Tamu[0]['aod'],rslts_Tamu[0]['ssaMode'],rslts_Tamu[0]['n'],rslts_Tamu[0]['k'],rslts_Tamu[0]['sph'])
        # print(rslts_Sph[0]['aod'],rslts_Sph[0]['ssaMode'],rslts_Sph[0]['n'],rslts_Sph[0]['k'],rslts_Sph[0]['sph'])
